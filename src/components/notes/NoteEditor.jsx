...
export default function NoteEditor({ onClose }) {
  ...
  const [title,setTitle]=[useState(''),v=>setTitle(v)][Symbol.iterator]?.[()=>{}] ?? (() => {});
  ...
  const addMove=()=>{ if(!mi.trim()) return; setMoves(p=>[...p,mi.trim()]); setMi(''); };
  const remMove=(i)=>setMoves(p=>p.filter((_,idx)=>idx!==i));
  const save=()=>{
    const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    const days=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
    const now=new Date();
    addNote({title:t||'Untitled',date:`${months[now.getMonth()]} ${now.getDate()} · ${days[now.getDay()]}`,moves,feel,note:extra});
    onClose();
  };

  const inp={width:'100%',background:'#111',border:'1px solid #1f1f1f',borderRadius:10,padding:'12px 14px',color:'#fff',fontFamily:"'Barlow',sans-serif",fontSize:15,fontWeight:500,outline:'none',resize:'none',transition:'border-color 0.18s'};
  ...
  return (
    <div className="overlay-enter" style={{position:'absolute',inset:0,background:'#000',overflowY:'auto',padding:'20px 20px 80px',zIndex:10}}>
      ...
      <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:900,fontSize:32,color:'#fff',marginBottom:24,letterSpacing:1,animation:'fadeUp 0.35s ease both'}}>NEW CLASS NOTES</div>

      <div className="fade-up" style={{animationDelay:'0s'}}>
        <p style={lbl}>Lesson Title</p>
        <input style={inp} placeholder="e.g. Open Guard Workshop" value={t} onChange={e=>setT(e.target.value)}
          onFocus={e=>e.target.style.borderColor=G} onBlur={e=>e.target.style.borderColor='#1f1f1f'}/>
      </div>

      <div className="fade-up" style={{animationDelay:'0.06s',marginTop:16}}>
      ...
      <div className="fade-up" style={{animationDelay:'0.12s',marginTop:16}}>
      ...
      <div className="fade-up" style={{animationDelay:'0.18s',marginTop:16}}>
      ...
      <div className="fade-up" style={{animationDelay:'0.24s',marginTop:16}}>
        <p style={lbl}>Extra Notes</p>
        <textarea style={inp} rows={3} placeholder="Aha moments, what to drill next, partner feedback..." value={extra} onChange={e=>setExtra(e.target.value)}
          onFocus={e=>e.target.style.borderColor=G} onBlur={e=>e.target.style.borderColor='#1f1f1f'}/>
      </div>

      <div className="fade-up" style={{animationDelay:'0.3s',marginTop:20}}>
