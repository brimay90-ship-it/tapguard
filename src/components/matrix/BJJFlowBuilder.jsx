  ...
  return (
    <div style={{
              ...
              <NodeAt key={`g-${nb.id}-${gtCode}`} dx={px} dy={py} z={2} delay={i*40+ghostIndex*20+180}
                ...
                <div style={{
                  ...
                  <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:11, fontWeight:700,
                    color:"#999", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{gt.name}</div>
                  <div style={{ fontSize:8, color:gc, fontWeight:700, textTransform:"uppercase",
                    letterSpacing:"0.05em", marginTop:1 }}>{gt.subcategory}</div>
                </div>
              </NodeAt>
            );
          });
              ...
              <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                ...
                {prof>0 && (
                  <span style={{ marginLeft:"auto", fontSize:10, fontWeight:800,
                    color:profColor(prof), fontFamily:"'Barlow Condensed',sans-serif", flexShrink:0 }}>{prof}</span>
                )}
              </div>
              <div style={{
      ...
      <NodeAt dx={0} dy={0} z={10} delay={0} noFade={fromId != null} onClick={slideOut ? null : ()=>onOpenDetail(focusId)}>
        <div style={{ position:"absolute", inset:-14, borderRadius:26,
          ...
          <div style={{ padding:"12px 14px 12px" }}>
            ...
            {(store.techs?.[focusId]?.proficiency||0) > 0 && (
              <div style={{ height:3, background:"#1a1a1a", borderRadius:2, overflow:"hidden", marginBottom:9 }}>
                <div style={{ height:"100%", width:`${(store.techs[focusId].proficiency||0)*10}%`,
                  background:col, opacity:0.6, borderRadius:2 }}/>
              </div>
            )}
            <div style={{ display:"flex", gap:6 }}>
              <div style={{ flex:1, textAlign:"center", padding:"8px 0", borderRadius:8,
                background:`${col}22`, fontSize:11, fontWeight:700, color:col,
                fontFamily:"'DM Sans',sans-serif" }}>Info ›</div>
              <div style={{ padding:"8px 10px", borderRadius:8, background:"#1e1e1e",
      ...
      <div style={{ padding:"8px 18px 12px", borderBottom:"1px solid #191919" }}>
        <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:10, color:col, fontWeight:700, textTransform:"uppercase",
              letterSpacing:"0.09em", marginBottom:3 }}>{tech.category}</div>
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif",
              fontSize:22, fontWeight:700, color:"#f5f5f5", lineHeight:1.1 }}>{tech.name}</div>
            {tech.aka?.length>0 && (
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:14 }}>
              {[
                ...
                <span key={i} style={{ fontSize:11, padding:"3px 10px", borderRadius:20,
                  background:b.c+"1e", color:b.c, fontWeight:600 }}>{b.l}</span>
              ))}
            </div>
            {tech.notes && (
              <p style={{ fontSize:13, color:"#666", lineHeight:1.7, marginBottom:14,
                borderLeft:`2px solid ${col}55`, paddingLeft:12 }}>{tech.notes}</p>
            )}
            {tech.common_setups?.length > 0 && (
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:10, fontWeight:700, color:"#22c55e", textTransform:"uppercase",
                  letterSpacing:"0.08em", marginBottom:8 }}>Setups</div>
                {tech.common_setups.map((s,i) => (
                  <div key={i} style={{ fontSize:13, color:"#999", padding:"6px 0",
                    borderBottom:"1px solid #181818", lineHeight:1.55 }}>• {s}</div>
                ))}
              </div>
            )}
            {tech.primary_counters?.length > 0 && (
              <div style={{ marginBottom:14 }}>
                <div style={{ fontSize:10, fontWeight:700, color:"#e53e3e", textTransform:"uppercase",
                  letterSpacing:"0.08em", marginBottom:8 }}>Counters</div>
                {tech.primary_counters.map((c,i) => (
                  <div key={i} style={{ fontSize:13, color:"#999", padding:"6px 0",
                    borderBottom:"1px solid #181818", lineHeight:1.55 }}>• {c}</div>
                ))}
              </div>
            )}
            {tech.notable_practitioners?.length > 0 && (
              <div>
                <div style={{ fontSize:10, fontWeight:700, color:"#d97706", textTransform:"uppercase",
                  letterSpacing:"0.08em", marginBottom:8 }}>Known for</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  {tech.notable_practitioners.map((p,i) => (
                    <span key={i} style={{ fontSize:12, padding:"3px 10px", background:"#181818",
                      borderRadius:20, color:"#bbb", border:"1px solid #202020" }}>{p}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div>
            <div style={{ marginBottom:18 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:10 }}>
                ...
                <span style={{ fontSize:32, fontWeight:800, color:profColor(prof),
                  fontFamily:"'Barlow Condensed',sans-serif" }}>
              ...
              <div style={{ height:3, background:"#181818", borderRadius:2, marginTop:8, overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${prof*10}%`, background:profColor(prof),
...
function Library({ onSelect, onBack }) {
  ...
  return (
    <div style={{ position:"absolute", inset:0, background:"#0a0a0a", zIndex:400, display:"flex", flexDirection:"column" }}>
      <div style={{ padding:"14px 14px 0", background:"#0d0d0d", borderBottom:"1px solid #191919", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
          <button onClick={onBack} style={{ background:"#181818", border:"none", color:"#999",
...
function Arsenal({ byId, onBack, onLoadFlow, onNavigate }) {
  ...
  return (
    <div style={{ position:"absolute", inset:0, background:"#0a0a0a", zIndex:400, display:"flex", flexDirection:"column" }}>
      <div style={{ padding:"14px 16px 12px", background:"#0d0d0d",
        ...
        <div style={{ display:"flex", gap:6, marginBottom:10, flexWrap:"wrap" }}>
          {[
            ...
            <span key={i} style={{ fontSize:11, padding:"3px 10px", borderRadius:20,
              background:b.c+"1e", color:b.c, fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}>{b.l}</span>
          ))}
        </div>
        <div style={{ display:"flex", gap:6 }}>
                ...
                <div key={cat} style={{ marginBottom:22 }}>
                  ...
                  {items.sort((a,b)=>b.info.proficiency-a.info.proficiency).map(({t,info}) => (
                    <div key={t.id}
                      ...
                      <div style={{ fontSize:20, fontWeight:800, color:profColor(info.proficiency),
                ...
                <div key={f.id} style={{ background:"#111", borderRadius:14,
                  ...
                  <div style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:10 }}>
                    ...
                    <button onClick={()=>{onLoadFlow(f);onBack();}} style={{
                      ...
                      fontFamily:"'DM Sans',sans-serif", flexShrink:0 }}>Load</button>
                    <button onClick={()=>delFlow(f.id)} style={{
                      ...
                      color:"#333", cursor:"pointer", fontSize:12, flexShrink:0 }}>✕</button>
                  </div>
                  {/* Chain preview */}
                      ...
                      return (
                        <div key={id} style={{ display:"flex", alignItems:"center", flexShrink:0 }}>
                          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
                            ...
                            <span style={{ fontSize:9, color:tc,
    ...
    <div style={{ position:"fixed", inset:0, background:"#000b",
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <input value={name} onChange={e=>setName(e.target.value)} autoFocus
          ...
          style={{ width:"100%", boxSizing:"border-box", padding:"13px 14px",
    ...
    <div style={{ position:"absolute", inset:0, background:"#0a0a0a",
              ...
              return (
                <button key={s.id} onClick={()=>{haptic(8);onSelect(s.id);}} style={{
                  ...
                  <div style={{ fontSize:14, fontWeight:700, color:"#f0f0f0",
                    fontFamily:"'Barlow Condensed',sans-serif", marginBottom:2 }}>{s.label}</div>
                  <div style={{ fontSize:10, color:"#666", fontFamily:"'DM Sans',sans-serif", lineHeight:1.2, flex:1 }}>{s.desc}</div>
                  <div style={{ fontSize:9, color:col, fontWeight:700, textTransform:"uppercase",
                    letterSpacing:"0.06em", marginTop:6 }}>{s.cat}</div>
                </button>
              );
            })}
          </div>
      ...
      <div style={{ padding:"16px 0", flexShrink:0 }}>
        <div onClick={onSearch} style={{
          ...
          <span style={{ fontSize:16, marginRight:10, opacity:0.6 }}>🔍</span>
          <span style={{ fontSize:14, color:"#666", fontFamily:"'DM Sans',sans-serif" }}>
...
function BeltPicker({ onSelect }) {
  ...
  return (
    <div style={{ position:"absolute", inset:0, background:"#0a0a0a", zIndex:500,
      display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
      padding:"32px 28px" }}>
      <div style={{ width:40, height:40, borderRadius:10, marginBottom:20,
        background:"linear-gradient(135deg,#22c55e,#16a34a)",
        display:"flex", alignItems:"center", justifyContent:"center",
        fontSize:20, fontWeight:900, color:"#fff", fontFamily:"'Barlow Condensed',sans-serif" }}>T</div>
      <div style={{ fontFamily:"'Barlow Condensed',sans-serif",
        fontSize:28, fontWeight:700, color:"#f0f0f0", marginBottom:6, textAlign:"center" }}>
        What's your belt?
      </div>
      <div style={{ fontSize:13, color:"#444", marginBottom:32, textAlign:"center",
        fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ flex:1, position:"relative", overflow:"hidden" }}>
        {focusId ? (
          ...
          <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column",
            ...
            <div style={{ fontSize:13, color:"#2a2a2a", textAlign:"center",
