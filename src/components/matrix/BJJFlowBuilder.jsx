      ...
      <div 
        ...
        {tab === "train" && (
          <div>
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-pri)', fontFamily: "'Barlow Condensed',sans-serif", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 12 }}>Training Notes</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
              <div style={{ 
                padding: "12px 14px", borderRadius: 12, background: "var(--bg-total)", 
                border: "1px solid var(--border)", minHeight: 44 
              }}>
                <div style={{ fontSize: 9, color: "#22c55e", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>What was covered</div>
                <textarea 
                  value={notesCovered} 
                  ...
                  style={{
                    ...
                    resize: "none", height: 60, lineHeight: 1.5, outline: "none", padding: 0
                  }}
                />
              </div>
              <div style={{ 
                ...
                <div style={{ fontSize: 9, color: "#3b82f6", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>Extra Notes</div>
                <textarea 
                  value={notesExtra} 
            ...
            <button onClick={() => { 
              // 1. Matrix local save
              rate(tech.id, prof, status, notesCovered, notesExtra); 
              
              // 2. Sync to Main Journal
              const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
              const days   = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
              const now    = new Date();
              const dateStr = `${months[now.getMonth()]} ${now.getDate()} · ${days[now.getDay()]}`;
              
              const existingMatch = notes.find(n => n.date === dateStr && (n.title.toLowerCase().includes(tech.name.toLowerCase()) || n.moves.includes(tech.name)));
              
              if (existingMatch) {
                updateNote(existingMatch.id, {
                  notesCovered,
                  notesExtra,
                  moves: Array.from(new Set([...existingMatch.moves, tech.name]))
                });
              } else {
                addNote({
                  title: `Technique: ${tech.name}`,
                  date: dateStr,
                  moves: [tech.name],
                  feel: prof >= 8 ? 'locked' : prof >= 5 ? 'good' : prof >= 3 ? 'okay' : 'lost',
                  notesCovered,
                  notesExtra,
                  category: 'Techniques'
                });
